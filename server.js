...
// Función para enviar mensaje de Twilio - CORREGIDA
async function sendTwilioMessage(playerName, action = 'conectado') {
    // Solo enviar si está configurado Twilio
    if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN || !process.env.TWILIO_TO_PHONE_NUMBER) {
        console.log('Twilio no configurado, saltando envío de mensaje');
        return;
    }

    try {
        const messageBody = `🎮 El jugador ${playerName} está ${action} en la MESA GOLD 5.000`;
        
        const message = await twilioClient.messages.create({
            to: process.env.TWILIO_TO_PHONE_NUMBER,  // Cambiado: usar 'to' en lugar de 'messagingServiceSid'
            messagingServiceSid: process.env.TWILIO_MESSAGING_SERVICE_SID,
            body: messageBody
        });
        
        console.log(`✅ Mensaje de Twilio enviado: ${message.sid} - "${messageBody}"`);
        return message;
    } catch (error) {
        console.error('❌ Error enviando mensaje de Twilio:', error.message);
...
if (serviceAccount) {
   admin.initializeApp({
       ...
       databaseURL: process.env.FIREBASE_DATABASE_URL || "https://juego-memoria-sexto-default-rtdb.firebaseio.com"
   });
   db = admin.database();
...
// Datos de usuario actualizados para el servidor 5 con nombres diferentes
const users = [
   { id: '1', username: 'Leon', password: 'fuerza987', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
   { id: '2', username: 'Falcon', password: 'vuelo654', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
   { id: '3', username: 'Tiburon', password: 'oceano321', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
   { id: '4', username: 'Condor', password: 'altura852', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
   { id: '5', username: 'Lynx', password: 'salvaje159', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
   { id: '6', username: 'Puma', password: 'velocidad753', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
   { id: '7', username: 'Coyote', password: 'astuto426', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
   { id: '8', username: 'Serpiente', password: 'sigilosa817', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
   { id: '9', username: 'Rino', password: 'fuerza294', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
   { id: '10', username: 'Elefante', password: 'gigante685', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
...
io.on('connection', (socket) => {
   ...
   socket.on('reconnectUser', async ({ userId, username }) => {  // ← Añadir async aquí
       ...
       if (playerIndex !== -1) {
           ...
           // *** NUEVO: Enviar mensaje de Twilio cuando se reconecta ***
           if (!wasConnected) {
               await sendTwilioMessage(username, 'reconectado');
           }
   ...
   // Unirse al juego
   socket.on('joinGame', async () => {  // ← Añadir async aquí
       ...
       if (existingPlayerIndex === -1) {
           ...
           // *** NUEVO: Enviar mensaje de Twilio cuando se conecta un jugador ***
           await sendTwilioMessage(user.username, 'en línea');
   ...
   // Desconexión
   socket.on('disconnect', async () => {  // ← Añadir async aquí
       ...
       if (userId) {
           ...
           // *** NUEVO: Enviar mensaje de Twilio cuando se desconecta ***
           if (user && !user.isAdmin) {  // No enviar para admins
               await sendTwilioMessage(user.username, 'desconectado');
           }
...
console.log('URL del cliente:', process.env.CLIENT_URL || 'https://juegomemoriacliente5-dfzg.onrender.com');
...
console.log('Base de datos Firebase:', 'https://juego-memoria-sexto-default-rtdb.firebaseio.com');
...
server.listen(PORT, () => {
   ...
   console.log('=================================');
   console.log('SERVIDOR 5 - JUEGO MEMORIA SEXTO');
   console.log('=================================');
   console.log('Usuarios disponibles:');
   console.log('- Leon (fuerza987)');
   console.log('- Falcon (vuelo654)');
   console.log('- Tiburon (oceano321)');
   console.log('- Condor (altura852)');
   console.log('- Lynx (salvaje159)');
   console.log('- Puma (velocidad753)');
   console.log('- Coyote (astuto426)');
   console.log('- Serpiente (sigilosa817)');
   console.log('- Rino (fuerza294)');
   console.log('- Elefante (gigante685)');
...
app.get('/', (req, res) => {
   res.send('Servidor del juego de memoria (Servidor 5 - juego-memoria-sexto) funcionando correctamente');
});
